#!/usr/bin/env bash
# Build a white-label customer APK for a specific tenant.
#
# Usage:
#   ./scripts/build_customer.sh \
#     --app-token "tok_abc123" \
#     --api-url "https://store.example.com/api/v1" \
#     --app-name "Jeyam Mutton" \
#     --app-id "com.jeyammutton.customer" \
...
echo "Building CUSTOMER APK for: $APP_NAME"
echo "  API URL: $API_URL"
echo "  App Mode: customer"
...
echo ""
echo "Customer APK built: build/app/outputs/flutter-apk/app-release.apk"

# Rename with tenant identifier if app-id provided
if [ -n "$APP_ID" ]; then
  SAFE_NAME=$(echo "$APP_ID" | tr '.' '-')
  cp build/app/outputs/flutter-apk/app-release.apk \
     "build/app/outputs/flutter-apk/${SAFE_NAME}-customer.apk"
  echo "Copied to: build/app/outputs/flutter-apk/${SAFE_NAME}-customer.apk"
fi
